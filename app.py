...
song_cluster_pipeline = joblib.load('Website/Model/song_cluster_pipeline_21.pkl')
...
def index():
    if request.method == 'POST':
        song_data = request.form.getlist('songs')
        songs = []
        for song in song_data:
            name, year = song.split(',')
            songs.append({'name': name.strip(), 'year': int(year.strip())})
        recommended_songs = recommend_songs(songs, data)
        return render_template('index.html', recommended_songs=recommended_songs)

    return render_template('index.html', recommended_songs=None)
